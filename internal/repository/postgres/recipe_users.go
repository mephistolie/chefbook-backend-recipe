...
func (r *Repository) GetRecipeBook(userId uuid.UUID) ([]entity.RecipeState, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE
			%[2]v.user_id=$1 AND (%[1]v.owner_id=$1 OR %[1]v.visibility<>'%[5]v')
	`, recipesTable, recipeUsersTable, scoresTable, getRecipeCollectionIdsSubquery, model.VisibilityPrivate)
